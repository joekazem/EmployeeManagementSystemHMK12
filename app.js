...
const startQuestions = ['View all Employees', 'View all Emplyees by Department', 'View budget by Department', 'View all Employees by Manager', 'Add Employee', 'Remove Employee', 'Update Employee Role', 'View all Roles', 'Add Role', 'Remove Role', 'View all Departments', 'Add Department', 'Remove Department', 'Exit']
...
const showByDept = () => {
    ...
    connection.query(deptQuery, (err, results) => {
        ...
        inquirer.prompt([
            ...
            connection.query(query, { department_name: chosenDept.department_name }, (err, results) => {
                if (err) throw err;
                //console.log(err);
                console.table(`All Employees by Department: ${chosenDept.department_name}`, results)
                runApp();
            })
        })
    })
...
const viewDeptBudget= () => {
    ...
    connection.query(selectDepartments, (err, results) => {
        if (err) throw err;

        inquirer.prompt([
            {
                name: 'deptBudgetChoice',
                type: 'list',
                choices: function () {
                    let choiceArray = results.map(choice => choice.department_name)
                    return choiceArray;
            ...
            let chosenDept;
            for (let i = 0; i < results.length; i++) {
                if (results[i].department_name === answer.deptBudgetChoice) {
                    chosenDept = results[i];
                }
            }

            const query = 'SELECT d.department_name AS "Department", SUM(r.salary) AS "Total Budget" FROM employees e INNER JOIN roles r ON r.id = e.role_id INNER JOIN departments d ON d.id = r.department_id WHERE d.?;';
            connection.query(query, { department_name: chosenDept.department_name }, (err, results) => {
                if (err) throw err;
                //console.log(err);
